...
export const ProductCategoryStatusToggle = () => {
  // Hooks 一律放最上面
  const record = useRecordContext<ProductCategoryRow>();
  const dataProvider = useDataProvider();
  const notify = useNotify();
  const refresh = useRefresh();
  const [loading, setLoading] = useState(false);

  // RBAC：ROLE_ADMIN / ROLE_SUPER_ADMIN 或具 product:edit 權限時允許切換狀態
  const roles = getStoredAuthRoles();
  const canToggle = hasRoleAdmin(roles) || hasStoredAuthority(roles, "product:edit");
  ...
  const handleToggle = async () => {
    ...
    try {
      await dataProvider.update("product_categories", {
        id: record.id,
        data: {},
        meta: {
