...
// 假設您已經建立好對應的 Hook 檔案
import type { APSummaryReportDto } from "@/hooks/useAPSummaryReport";
...
interface APSummaryReportTableProps {
...
export const APSummaryReportTable = ({
  data,
  loading,
  error,
  onRetry,
}: APSummaryReportTableProps) => {
  ...
  const previousDataRef = useRef<APSummaryReportDto[]>(data);
  const [displayData, setDisplayData] = useState<APSummaryReportDto[]>(data);
    ...
    <Card>
      <CardContent>
        {error ? (
          ...
          <Alert severity="info">目前沒有符合條件的應付帳款數據</Alert>
        ) : (
          <Box
            sx={{
              position: "relative",
              ...
              <Table stickyHeader>
                <TableHead>
                  ...
                  >
                    ...
                    <SortableHeader field="totalPayable">
                      總應付金額
                    </SortableHeader>
                ...
                <TableBody>
                  {sortedDataRows.map((row) => (
                    <TableRow key={row.accountingPeriod} hover>
                      ...
                      <TableCell align="right">
                        <PlainCurrency value={row.totalPayable} showDecimal />
                      </TableCell>
                      
                      <TableCell align="right" sx={{ color: 'text.secondary' }}>
                        <PlainCurrency value={row.totalPaid} showDecimal />
                      </TableCell>
                  ...
                  {summaryRow && (
                    <TableRow
                      ...
                      sx={{
                        backgroundColor: isDark
                          ? "rgba(198, 40, 40, 0.15)" // 使用淡紅色背景區分 (AP合計較為警示)
                          : "rgba(198, 40, 40, 0.05)",
                        "& td": {
                          fontWeight: 600,
                    ...
                    >
                      ...
                      <TableCell align="right">
                        <PlainCurrency value={summaryRow.totalPayable} showDecimal />
                      </TableCell>
                      <TableCell align="right">
                        <PlainCurrency value={summaryRow.totalPaid} showDecimal />
                      </TableCell>
