...
// 確保您已安裝 @tanstack/react-query (React Admin v4/v5 內建或需額外安裝)
...
export interface ARSummaryReportDto {
  ...
  totalReceivable: number;  // 總應收帳款
  totalReceived: number;    // 已收帳款
  totalOutstanding: number; // 未收餘額
}
...
export const useARSummaryReport = (params: ARSummaryQueryParams = {}) => {
  ...
  const queryKey = useMemo(() => {
    ...
    // 只有當有參數時才回傳 key，否則回傳 null (disabled)
    if (normalizedPeriods.length === 0 && !normalizedPeriod) {
        return ['arSummary', 'empty'];
    }

    return ['arSummary', { periods: normalizedPeriods, period: normalizedPeriod, endDate: normalizedEndDate }];
