...
/**
 * 對應後端 ARSummaryReportDto
 * 欄位必須與 Java DTO 完全一致
 */
export interface ARSummaryReportDto {
  accountingPeriod: string; // 會計期間 (YYYY-MM 或 YYYY-MM-DD)
  totalReceivable: number;  // 總應收帳款 (訂單總額)
  totalReceived: number;    // 已收帳款 (PAID)
  totalOutstanding: number; // 未收餘額 (UNPAID)
}

export interface ARSummaryQueryParams {
...
const CACHE_KEY_PREFIX = "ar_summary_report_cache_";
...
// 生成緩存鍵
const getCacheKey = (params: ARSummaryQueryParams): string => {
  // 優先使用 periods (多月份)
...
const getCachedData = (
  ...
  try {
    ...
    console.warn("讀取 AR Summary 緩存失敗：", error);
    return null;
  }
};
...
const setCachedData = (
  ...
  try {
    ...
    console.warn("保存 AR Summary 緩存失敗：", error);
  }
};
...
/* =========================================================
 * Hook: useARSummaryReport
 * ========================================================= */

export const useARSummaryReport = (
  params: ARSummaryQueryParams = {}
) => {
  const dataProvider = useDataProvider();
  const dataProviderRef = useRef(dataProvider);
  const [data, setData] = useState<ARSummaryReportDto[]>([]);
  ...
  const fetchData = useCallback(
    async (skipCache = false, retryCount = 0): Promise<void> => {
      ...
      try {
        ...
        // 3. 發送請求
        // 假設後端資源路徑為 reports/ar_summary
        const resourcePath = "reports/ar_summary"; 
        ...
        // 4. 數據處理 (確保轉為 Array)
        let reportData: ARSummaryReportDto[] = [];
        if (response.data) {
          if (Array.isArray(response.data)) {
            reportData = response.data as ARSummaryReportDto[];
        ...
        if (retryCount < MAX_RETRY_ATTEMPTS) {
          console.warn(`載入 AR 報表失敗，重試中 (${retryCount + 1}/${MAX_RETRY_ATTEMPTS})`);
          await delay(RETRY_DELAY * (retryCount + 1));
          return fetchData(skipCache, retryCount + 1);
        }

        // 錯誤處理
        console.error("❌ 載入 AR 報表失敗：", err);
